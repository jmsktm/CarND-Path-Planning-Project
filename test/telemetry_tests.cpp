...
TEST_CASE("telemetry.cpp >> _hasEventMessageData()") {
    Vehicle vehicle;
    REQUIRE_FALSE(Telemetry("", vehicle)._hasEventMessageData());
    REQUIRE_FALSE(Telemetry("test", vehicle)._hasEventMessageData());
    REQUIRE_FALSE(Telemetry("42[]", vehicle)._hasEventMessageData());
    REQUIRE(Telemetry("42[\"telemetry\",{}]", vehicle)._hasEventMessageData());
}

TEST_CASE("telemetry.cpp >> _getEventMessageData() with valid input data") {
    Vehicle vehicle;
    string result = Telemetry("42[\"telemetry\",{\"a\":\"b\"}]", vehicle)._getEventMessageData();
    REQUIRE(result == "[\"telemetry\",{\"a\":\"b\"}]");
}

TEST_CASE("telemetry.cpp >> _hasTelemetryData()") {
    Vehicle vehicle;
    bool falseCase = Telemetry("42[\"test\",{\"a\":\"b\"}]", vehicle)._hasTelemetryData();
    REQUIRE_FALSE(falseCase);

    bool trueCase = Telemetry("42[\"telemetry\",{\"a\":\"b\"}]", vehicle)._hasTelemetryData();
    REQUIRE(trueCase);
}
...
TEST_CASE("telemetry.cpp >> _getTelemetryData()") {
    SECTION("Does not contain telemetry data") {
        Vehicle vehicle;
        json result = Telemetry("42[\"test\",{\"a\":\"b\"}]", vehicle)._getTelemetryData();
        json expected = json({});
        REQUIRE(result == expected); 
    }
    SECTION("Contains telemetry data") {
        Vehicle vehicle;
        json result = Telemetry("42[\"telemetry\",{\"a\":\"b\"}]", vehicle)._getTelemetryData();
        json expected = R"(
            {
                "a": "b"
            }
...
TEST_CASE("Get properties from the (valid) telemetry data") {
    ...
    Vehicle vehicle;
    Telemetry telemetry = Telemetry("42[\"telemetry\"," + str + "]", vehicle);
...
TEST_CASE("Return last two points when previous path is not available") {
    ...
    Vehicle vehicle;
    Telemetry telemetry = Telemetry(data_string, vehicle);
    Vehicle ego_vehicle = telemetry.get_ego_vehicle();
    REQUIRE(ego_vehicle.get_x() == 1.0);
    REQUIRE(ego_vehicle.get_y() == 2.0);
    REQUIRE(ego_vehicle.get_s() == 3.0);
...
TEST_CASE("Test I: Convert from world frame coordinates to car frame coordinates") {
    // Rounding off to account for unpredictably changing precision
    #define roundz(x,d) ((floor(((x)*pow(10,d))+.5))/pow(10,d))

    vector<double> world_frame_x = { roundz(5.0, 3), roundz(6.0, 3), roundz(7.0, 3), roundz(8.0, 3), roundz(8.0, 3) };
    vector<double> world_frame_y = { roundz(1.0, 3), roundz(0.0, 3), roundz(0.0, 3), roundz(2.0, 3), roundz(3.0, 3) };
    tuple<vector<double>, vector<double>> world_frame_coordinates = std::make_tuple(world_frame_x, world_frame_y);

    Vehicle vehicle;
    Telemetry telemetry = Telemetry("42[\"telemetry\",{}]", vehicle);
    ...
    vector<double> expected_x = { roundz(-1.414, 3), roundz(0.0, 3), roundz(0.707, 3), roundz(0.0, 3), roundz(-0.707, 3) };
    vector<double> expected_y = { roundz(0.0, 3), roundz(0.0, 3), roundz(0.707, 3), roundz(2.828, 3), roundz(3.536, 3) };

    REQUIRE(actual_x == expected_x);
    REQUIRE(actual_y == expected_y);
}

TEST_CASE("Test II: Convert from world frame coordinates to car frame coordinates") {
    // Rounding off to account for unpredictably changing precision
    #define roundz(x,d) ((floor(((x)*pow(10,d))+.5))/pow(10,d))

    vector<double> world_frame_x = { roundz(932.047, 3), roundz(932.486, 3), roundz(949.291, 3), roundz(979.535, 3), roundz(1009.1, 3) };
    vector<double> world_frame_y = { roundz(1129.16, 3), roundz(1129.22, 3), roundz(1130.59, 3), roundz(1135.55, 3), roundz(1144.62, 3) };
    tuple<vector<double>, vector<double>> world_frame_coordinates = std::make_tuple(world_frame_x, world_frame_y);
    ...
    vector<double> expected_x = { roundz(-0.443, 3), roundz(0.0, 3), roundz(16.836, 3), roundz(47.473, 3), roundz(77.994, 3) };
    vector<double> expected_y = { roundz(0.0, 3), roundz(0.0, 3), roundz(-0.918, 3), roundz(-0.099, 3), roundz(4.883, 3) };
...
TEST_CASE("ego vehicle should be set on instantiation of Telemetry") {
    ...
    Vehicle vehicle;
    Telemetry telemetry = Telemetry(data_string, vehicle);
    vehicle = telemetry.get_ego_vehicle();
